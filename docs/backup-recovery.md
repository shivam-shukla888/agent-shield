# AgentShield — Database Backup & Recovery Guide

## 1. PostgreSQL Backup Strategy

AgentShield persists security scan history and results in PostgreSQL (`scans` table). To ensure business continuity and disaster recovery, production deployments must establish automated database backups.
...
pg_dump -h $DB_HOST -U $DB_USER -d $DB_NAME -F c -b -v -f "/backups/agentshield_$(date +%Y%m%d_%H%M%S).dump"
```

### 2.2 Containerized Docker Backup

```bash
docker exec -t agentshield-postgres pg_dump -U agentshield -d agentshield -F c > backup.dump
```
...
2. Terminate active application connections to prevent write conflicts:
   ```sql
   SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'agentshield' AND pid <> pg_backend_pid();
   ```
...
4. Verify table integrity:
   ...
   ```
5. Restart AgentShield API service:
   ```bash
...
The AgentShield PostgreSQL schema is managed automatically on service startup via `init_db()`.
